...
- **`mac-vrf` instance VLAN scope** - Batfish's Juniper grammar does not model VLAN definitions placed inside a `routing-instances X { instance-type mac-vrf; vlans { ... } }` block. Two gaps in [`FlatJuniper_routing_instances.g4`](https://github.com/batfish/batfish/blob/master/projects/batfish/src/main/antlr4/org/batfish/grammar/flatjuniper/FlatJuniper_routing_instances.g4) cause it: the `ri_instance_type` rule has no `MAC_VRF` token (accepts only `forwarding | l2vpn | virtual-router | virtual-switch | vrf`), and `ri_named_routing_instance` has no `vlans` child rule at all. VLAN names declared inside a mac-vrf never enter the Batfish symbol table, so `family ethernet-switching vlan members VLANX` shows up in `undefinedReferences` with `Struct_Type="vlan"`, and IRBs bound to those VLANs get deactivated. This is why `questions.py` carries `IGNORED_REF_STRUCT_TYPES = {"vlan"}` and the `IGNORED_INIT_ISSUE_PATTERNS` list. Tracked upstream in [batfish#5036](https://github.com/batfish/batfish/issues/5036) (Juniper VXLAN support, still open). The raw `initIssues()` / `undefinedReferences()` output the filters target is captured in [tests/fixtures/raw_init_issues_2026-04-11.txt](tests/fixtures/raw_init_issues_2026-04-11.txt); regenerate via [tests/fixtures/capture_raw_output.py](tests/fixtures/capture_raw_output.py) when the Batfish image bumps.
