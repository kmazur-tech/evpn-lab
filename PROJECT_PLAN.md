...
- Pipeline stages:
  ...
  11. **Suzieq drift check** - NetBox-vs-state diff (Phase 5 Python harness, `--mode assertions`). Hard fail in `fabric-deploy.yml`: a persistent drift past the 90 s + 4x45 s retry budget triggers `rollback-on-failure`. The earlier "soft fail = warn" plan was promoted to hard-fail when the marker-based outer rollback landed (Phase 6.3) -- soft-failing would hide deploy regressions that the smoke suite did not directly verify (anycast MAC, EVPN Type-2 ARP, peer VTEP learning).
